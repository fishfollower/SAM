...
obj <- MakeADFun(Fobs, par, random=c("logU","logV"), DLL="logUV", 
                 map=map)
fit <- nlminb(obj$par, obj$fn, obj$gr)
sdr<-sdreport(obj)

Fest = as.list(sdr,"Est")$logU
for(i in 1:dim(Fest)[2]){
  Fest[,i] = Fest[,i] + as.list(sdr,"Est")$logV[i]
}

matplot(Fobs$year, t(Fest), type="l", add=TRUE)
