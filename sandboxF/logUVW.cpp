  ...
  Type objective_function<Type>::operator() ()
{
  ...
  for(int y=0; y<nrow; ++y){
    for(int a=0; a<ncol; ++a){
      jnll += -dnorm(logFUpper(y,a),logF(y,a), sd, true); // Add noize which does not come from U or V.
    }
  }
  
  for(int y=0; y<nrow; ++y){
    for(int a=0; a<ncol; ++a){
      jnll += -dnorm(log(Fobs(y,a)), logFUpper(y,a), sdObs, true);
    }
  }
