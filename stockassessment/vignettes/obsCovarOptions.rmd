...
\vspace{2mm}
Let $\mu_{a,y}$ be the predicted observation for age $a$ at year $y$ on natural scale, and let $v_{a,y}$ be the correspondning variance. With this option we impose the assumption that
\begin{align}
...
```
Note that the estimated variances will be scaled internally in SAM when we provide the `cov-weight` attribute, and the correlation structure will be used as fixed. If we want to use the externally estiamted variances as fixed (probably never used), we rename the attribut to `cov`, i.e:
```r
