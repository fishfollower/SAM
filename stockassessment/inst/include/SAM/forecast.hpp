...
namespace forecast_fun {
  ...
  SAM_SPECIALIZATION(void extendArray_2D(array<double>&, int, int, vector<int>, bool));
  SAM_SPECIALIZATION(void extendArray_2D(array<TMBad::ad_aug>&, int, int, vector<int>, bool));
  ...
  SAM_SPECIALIZATION(void extendArray_3D(array<double>&, int, int, vector<int>, bool));
  SAM_SPECIALIZATION(void extendArray_3D(array<TMBad::ad_aug>&, int, int, vector<int>, bool));
...
Type hcr(Type ssb, vector<Type> hcrConf)SOURCE({
    ...
    if(fabs(Btrigger - Borigin) < 1e-12){
      ...
      newF = TMBad::CondExpLt(ssb,
			      Bcap,
			      Fcap,
			      forecast_fun::hcr_min(Ftarget, forecast_fun::hcr_max(Forigin, Forigin + (ssb - Borigin) * (Ftarget - Forigin) / (Btrigger - Borigin))));
    }
    return log(newF);	  
  });
...
void extendArray(array<Type>& x, int nModelYears, int nForecastYears, vector<int> aveYears, bool keepModelYears DEFARG(= true))SOURCE({
  if(x.dim.size() == 2){
    forecast_fun::extendArray_2D(x, nModelYears, nForecastYears, aveYears, keepModelYears);
  }else if(x.dim.size() == 3){
    forecast_fun::extendArray_3D(x, nModelYears, nForecastYears, aveYears, keepModelYears);
  }else{
