...
struct forecastSet {
  ...
  Type initialFbar;
	 ...
	 void updateForecast(int i, array<Type>& logF, array<Type>& logN, array<Type>& logitFseason, dataSet<Type>& dat, confSet& conf, paraSet<Type>& par, Recruitment<Type>& recruit, MortalitySet<Type>& mort);
...
SOURCE(
	 ...
	 void forecastSet<Type>::calculateForecast(array<Type>& logF, array<Type>& logN, array<Type>& logitFseason, dataSet<Type>& dat, confSet& conf, paraSet<Type>& par, Recruitment<Type>& recruit, MortalitySet<Type>& mort){
	   ...
	   for(int i = 0; i < nYears; ++i){
	     updateForecast(i, logF, logN, logitFseason, dat, conf, par, recruit, mort);
	   }
	   return;
	 }
	 )
...
SOURCE(
	 template <class Type>
	 void forecastSet<Type>::updateForecast(int i, array<Type>& logF, array<Type>& logN, array<Type>& logitFseason, dataSet<Type>& dat, confSet& conf, paraSet<Type>& par, Recruitment<Type>& recruit, MortalitySet<Type>& mort){
	   if(nYears == 0){ // no forecast
	     return;
	   }
