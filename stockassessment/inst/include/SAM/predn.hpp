...
vector<Type> predNFun(dataSet<Type> &dat, confSet &conf, paraSet<Type> &par, array<Type> &logN, array<Type> &logF, Recruitment<Type> &recruit, MortalitySet<Type>& mort, int i)SOURCE({
  ...
  if((i-conf.minAge)>=0){
    ...
    logThisSSB=erbi(dat,conf,par,logN,logF,mort,i-conf.minAge, true);    
  }else{
    // logThisSSB=ssbi(dat,conf,logN,logF,mort,0, true); // use first in beginning       
    logThisSSB=erbi(dat,conf,par,logN,logF,mort,0, true); // use first in beginning       
  }
  ...
  if(par.rec_transphi.size() > 0){
    ...
    for(int j = 1; j <= par.rec_transphi.size(); ++j){
      ...
      Type logThatSSB = erbi(dat,conf,par,logN,logF,mort,std::max(i-conf.minAge-j,0), true);
  ...
  for(int j=1; j<stateDimN; ++j){
    predN(j)=logN(j-1,i-1) - exp(mort.logCumulativeHazard(j-1,i-1)); //totF(j-1,i-1)-dat.natMor(i-1,j-1); 
  }
  if(conf.maxAgePlusGroup(0)==1){// plusgroup adjustment if catches need them 
    Type v1 = predN(stateDimN-1); // Already updated above
    Type v2 = logN(stateDimN-1,i-1) - exp(mort.logCumulativeHazard(stateDimN-1,i-1)); //totF(stateDimN-1,i-1) - dat.natMor(i-1,stateDimN-1); // Remaining in plus group from last year
