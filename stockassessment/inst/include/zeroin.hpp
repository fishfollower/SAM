...
/*
 *  2019 Christoffer Moesgaard Albertsen (cmoe@aqua.dtu.dk)
 *  Modified to work with TMB
 */
...
struct UNIROOT {
  ...
  UNIROOT() : tol(1e-6), maxit(1000) {};
  UNIROOT(Float tol_, int maxit_) : tol(tol_), maxit(maxit_) {};
  
  virtual Float operator()(Float x) = 0;

  Float R_zeroin2(Float ax, Float bx) {
    Float fa = this->operator()(ax);
    Float fb = this->operator()(bx);

    Float a,b,c, fc;			/* Abscissae, descr. see above,  f(c) */
    ...
    // Normally, this check is done on the R side
    if(!((fa < 0.0 && fb > 0.0) || (fb < 0.0 && fa > 0.0)))
      Rf_error("f() values at end points not of opposite sign");
	...
	if( fabs(prev_step) >= tol_act	/* If prev_step was large enough*/
	  ...
	  if( p>(Float)0 )		/* p was calculated with the */
	...
	b += new_step;	fb = this->operator()(b);	/* Do step to a new approxim. */
