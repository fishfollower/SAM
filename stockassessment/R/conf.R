...
saveConf <- function(x, file="", overwrite=FALSE){
  ...
  if(file.exists(file) & !overwrite){
    ...
    txt$keyBiomassTreat <- "To be defined only if a biomass survey is used (0 SSB index, 1 catch index, 2 FSB index, 3 total catch, 4 total landings, 5 TSB index and 6 TSN index)."
    ...
    txt$keyQprocess<-"Optionally added AR processes for catchbilities for surveys to allow time varying (highly experimental)" 
    nam<-names(x)
