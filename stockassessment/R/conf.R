...
defcon<-function(dat){
  ...
  ret$keyAgeSampleData <- rep(NA_integer_, nFleets)
  ret$offsetAgeSampleData <- c(0.5,dat$sampleTimes[-1])
  return(ret) 
}
...
saveConf <- function(x, file="", overwrite=FALSE){
  ...
  if(file.exists(file) & !overwrite){
    ...
    txt$keyAgeSampleData<-"An integer for each fleet indicating which data to use from the agesampledata matrix"
    txt$offsetAgeSampleData<-"Fraction into the age, where the ageConfusion is calculated"  
    nam<-names(x)
