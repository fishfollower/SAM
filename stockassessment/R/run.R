...
jit <- function(fit, nojit=10, par=defpar(fit$data, fit$conf), sd=.25, ncores=detectCores()){
  ...
  if(ncores>1){
    ...
    lib.loc <- dirname(path.package("stockassessment")) #location of loaded package, to prevent version conflicts
    clusterEvalQ(cl, {library(stockassessment, lib.loc=lib.loc)}) #load the package to each node
