...
.parseForecast <- function(s, FbarRange, fleetTypes, ageRange, useNonLinearityCorrection, isUpper=FALSE){
    ...
    parseOne <- function(ss){
        ...
        ## Find a way to check for input penalty for upper bound
        v <- list(specification = ss,
                  Amin = Amin,
                  Amax = Amax,
                  fleet = fleet,
    ...
    if(!isUpper && any(sapply(ft, function(x) any(x>1) || sum(x) > nCF)))
...
constraints[is.na(constraints) & !is.na(nextssb)] <- sprintf("SSB=%f",nextssb[is.na(constraints) & !is.na(nextssb)])
    ...
    if(addDataYears){
        lastIndx <- args$data$aux[,"year"] == max(args$data$aux[,"year"])
        lastAux <- args$data$aux[lastIndx,]
        newAuxL <- replicate(nYears,lastAux, simplify = FALSE)
