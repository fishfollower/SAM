...
defpar <- function(dat,conf){
  ...
  ret$logitReleaseSurvival=if(any(dat$aux[,2]==5)){numeric(length(conf$cutReleaseSurvival)+1)}else{numeric(0)}
  ret$logitRecapturePhi=if(any(dat$aux[,2]==5)){numeric(1)}else{numeric(0)}
