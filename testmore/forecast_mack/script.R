...
fv <- modelforecast(fit, fscale = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 0,processNoiseF=FALSE,year.base=max(fit$data$year))
...
## Test no error
fvs <- modelforecast(fit, fscale = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 10, progress=FALSE)

## Test fval
target <- c(0.1,0.2,0.3,0.1)
fv <- modelforecast(fit, fval = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 0,year.base=max(fit$data$year))
...
fv <- modelforecast(fit, catchval = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 0,year.base=max(fit$data$year))
...
fvs <- modelforecast(fit, catchval = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 10, progress=FALSE)

## Test landval
target <- round(c(0.1,0.1)*mean(tail(rep(catchtable(fit)[,1],4))))
fv <- modelforecast(fit, landval=target, biasCorrect = FALSE, splitLD = TRUE,ave.years = max(fit$data$years)+(-4:0), nosim = 0,year.base=max(fit$data$year))
...
fvs <- modelforecast(fit, landval = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 10, progress=FALSE, splitLD=TRUE)

## Test fmodel
target <- rep(NA,5)
fv <- modelforecast(fit, landval = target, biasCorrect = FALSE,ave.years = max(fit$data$years)+(-4:0), nosim = 0,year.base=max(fit$data$year))
